---
title: Why We Need Better Data Analyses in PSAPs
subtitle: Why are we leaving so much data on the table?
description: "null"
...
tags:
  ...
  - forecasting
  - staffing
image: images/posts/null-hero.png
image-alt: "null"
draft: true
toc: true
toc-depth: 3
...
- **Synthetic Data**: `data/synthetic/null.parquet` (generator in `src//generate_data.py`)
...
#| label: fig-main
#| echo: false
#| fig-cap: "Hourly call volume forecast vs actual — "
...
- **GitHub Repo**: `github.com/tonydunsworth/`
- **Data**: ☐ Public ☐ Synthetic ☐ Request access
- **Environment**: `renv.lock` / `requirements.txt` / `Project.toml` / `Cargo.lock` / `go.mod`
- **Render**: `quarto render .qmd --to html`
- **CI**: GitHub Actions → Netlify/Pages (auto-deploy on push to `main`)

```bash
# Local development
quarto preview why_we_need_better_analytics.qmd

# Freeze compute for faster re-renders
quarto render why_we_need_better_analytics.qmd --execute-freeze

# Deploy
git add . && git commit -m "post: why_we_need_better_analytics" && git push
```
...
*Template: Quarto 1.5+ | drddatascience.com/blog | `2026-09-09`*
