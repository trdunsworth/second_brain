...
subtitle: "<% tp.system.prompt('Subtitle (one compelling sentence):') %>"
...
categories: [<% tp.system.prompt('Categories (comma-separated, e.g., 911-dispatch, forecasting, staffing):') %>]
tags: [<% tp.system.prompt('Tags (comma-separated, e.g., nena, psap, time-series, staffing-model, duckdb, lightgbm):') %>]
...
- **Synthetic Data**: `data/synthetic/<% tp.system.prompt('Slug (URL-friendly):') %>.parquet` (generator in `src/<% tp.system.prompt('Slug (URL-friendly):') %>/generate_data.py`)
...
#| label: fig-main
#| echo: false
#| fig-cap: "Hourly call volume forecast vs actual — <% tp.system.prompt('Slug (URL-friendly):') %>"
...
- **GitHub Repo**: `github.com/tonydunsworth/<% tp.system.prompt('Slug (URL-friendly):') %>`
- **Data**: ☐ Public ☐ Synthetic ☐ Request access
- **Environment**: `renv.lock` / `requirements.txt` / `Project.toml` / `Cargo.lock` / `go.mod`
- **Render**: `quarto render <% tp.system.prompt('Slug (URL-friendly):') %>.qmd --to html`
- **CI**: GitHub Actions → Netlify/Pages (auto-deploy on push to `main`)

```bash
# Local development
quarto preview <% tp.system.prompt('Slug (URL-friendly):') %>.qmd

# Freeze compute for faster re-renders
quarto render <% tp.system.prompt('Slug (URL-friendly):') %>.qmd --execute-freeze

# Deploy
git add . && git commit -m "post: <% tp.system.prompt('Slug (URL-friendly):') %>" && git push
```
...
*Template: Quarto 1.5+ | drddatascience.com/blog | `<% tp.date.now('YYYY-MM-DD') %>`*
